...
static const headerType *QBGetProjectImageHeader() {
...
static classref_t *QBGetDataSection(const headerType *machHeader, const char *sectname, size_t *outCount) {
...
int QBGetProjectClassList(classref_t **classes) {
    size_t count = 0;
    if (!!classes) {
        *classes = QBGetDataSection(QBGetProjectImageHeader(), "__objc_classlist", &count);
    } else {
        QBGetDataSection(QBGetProjectImageHeader(), "__objc_classlist", &count);
    }
    return (int)count;
}

@implementation QBPropertyDescriptor

+ (instancetype)descriptorWithProperty:(objc_property_t)property {
    QBPropertyDescriptor *descriptor = [[QBPropertyDescriptor alloc] init];
    ...
    NSString *setterString = setterChar != NULL ? [NSString stringWithUTF8String:setterChar] : NSStringFromSelector(QBSetterWithGetter(NSSelectorFromString(propertyName)));
    ...
    descriptor.type = [QBPropertyDescriptor typeWithEncodeString:[NSString stringWithUTF8String:type]];
    if (type != NULL) {
        free(type);
    }
...
- (NSString *)description {
    ...
    if (self.setter != QBSetterWithGetter(NSSelectorFromString(self.name))) [result appendFormat:@", setter=%@", NSStringFromSelector(self.setter)];
    [result appendString:@") "];
